...
# Tested on JASMIN and blue pebble
...
# FGOALS-g3 has historical data that goes to 2016, I've added a manual fix for this model
...
# 26/11/2019 Fixed a bug regarding switching from 3D to 4D data. DMM
# 26/11/2019 Added an exception to catch models that have misnamed their dimensions DMM
# 03/12/2019 Added an exception to catch models with different id identifiers  DMM
# 03/12/2019 Should now read in anysize data file and .nc type DMM
# 03/12/2019 Added Iris functionality to signifiantly speed things up DMM
# 03/12/2019 Added ability to select certain lats DMM
# 31/03/2020 Changed directory structures to be applicable anywhere DMM
# 01/04/2020 Fixed bug when pressure levels were read in DMM
...
experiment = 'amip' # Could also be piControl, hist-GHG, hist-aer, hist-nat, amip, etc
members = 'first' # Either select 'all' or 'first'
model_name = 'all' #Either write a model name, or write 'all'
variable = 'ta'
project = 'CMIP' # only change this from CMIP if you want to look at non-DECK or ScenarioMIP experiments, e.g. DAMIP, HighResMIP etc

syear = 1979 # select data only after this year (and including it)
eyear = 2014 # ditto end year. If this year is after 2014 then RCP4.5-SSP2 data will be stitched on.

slat = -30 # select a latitue range, if required
elat = 30
...
# Path for data
path_input = '/bp1store/geog-tropical/data/CMIP6/'+project+'/' # Path to read in input files
path_input_obs = '/bp1store/geog-tropical/users/dm16883/Obs/Radiosondes/'
data_path = '/bp1store/geog-tropical/users/dm16883/Models/Radiosondes/' #'/home/users/dmitchell/Radiosonde/Data/Models/' # Path to write out final product
...
if (inter == True):
    path_obs = path_input_obs
    nc = ncfile(path_obs + 'raobcore17_gridded_2019.0.nc')
...
#np.save("pressure_obs_new",p_obs)
...
if (members == 'all'): ems = glob.glob(path_input+'*/'+model_name+'/'+experiment+'/r*i1p*f*/'+vtype+'/'+variable+'/'+gtype+'/')
if (members == 'first'): ems = glob.glob(path_input+'*/'+model_name+'/'+experiment+'/r1i1p1f*/'+vtype+'/'+variable+'/'+gtype+'/')
print(path_input+'*/'+model_name+'/'+experiment+'/r*i1p*f*/'+vtype+'/'+variable+'/'+gtype+'/')
print('There are ' + str(len(ems)) + ' models/ensemble members with the required data') 

if (eyear > 2014): 
    print('The end year exceeds the Historical simulations, tagging on SSP2-RCP4.5 data')
    project2 = 'ScenarioMIP'
    experiment2 = 'ssp245'
    path_input2 = '/bp1store/geog-tropical/data/CMIP6/'+project2+'/' # Path to read in input files for extension runs

    if (members == 'all'): ems_add = glob.glob(path_input2+'*/'+model_name+'/'+experiment2+'/r*i1p*f*/'+vtype+'/'+variable+'/'+gtype+'/')
    if (members == 'first'): ems_add = glob.glob(path_input2+'*/'+model_name+'/'+experiment2+'/r1i1p1f*/'+vtype+'/'+variable+'/'+gtype+'/')
    ...
    for count in np.arange(len(ems_add)): 
        ...
        #models_v2.append(glob.glob(ems_add[count]+'*'))

# Just take the latest version of the ensemble member
models_v1 = []

count_skip=0 # Makes script faster by testing dims only once in coe below
...
for count in np.arange(len(ems)): 
                     ...
                     #models_v1.append(glob.glob(ems[count]+'*'))

for count in np.arange(len(ems)): # Loop over the ensemble members and models
    
    #if (count == 12): continue # fudge to get past problematic model

    module_list = glob.glob(models_v1[count]+'/*')# for each model ensemble member, store all the data files
    if (len(module_list) == 0): 
        print('No files assosiated with: '+models_v1[count])
        continue # some models don't have any files, skip these
    #if (count == 14): continue #fudge for EC-Earth3, which doesn't have all the data
    #mident = glob.glob(models_v1[count]+'/*')[0][-50:-40]
    #module_list = models_v1[count]
    if (eyear > 2014): 
        nc=ncfile(module_list[0]) # very inefficeint 
        ...
        in_id = str(nc.institution_id)

        test_exist = models_v1[count][0:35]+project2+'/'+in_id+'/'+model_name+'/'+experiment2+'/'+ensemble_name+'/Amon/ta/'+grid_label+'/latest'
        print(model_name)
        if (os.path.isdir(test_exist) == True):
            for app_count in np.arange(len(glob.glob(test_exist+'/*'))):
                module_list.append(glob.glob(test_exist+'/*')[app_count])
        else: 
            print(test_exist + ' does not exist')
            continue #this skips out this models, I think!
            #module_list.append(glob.glob(models_v2[count]+'/*'))
        #module_list.append(models_v2[count])
    ...
    for count2 in np.arange(len(module_list)): # Select relevant years of the models and ensemble member
        ...
        if (syear <= file_s <= eyear) or (syear <= file_e <= eyear) or (file_s <= syear <= file_e): # Selects only files in the relevant range 
    ...
    #    print('File size too big for '+local_store[0])
    #    continue

    # Need to add a section here to include RCP data in the local_store

    lat_range = iris.Constraint(latitude=lambda cell: slat <= cell <= elat)
    time_range = iris.Constraint(time=lambda cell: syear <= cell.point.year <= eyear)

    data_subset = iris.load(local_store, lat_range & time_range)

    lat = data_subset[0].coord("latitude").points
    lon = data_subset[0].coord("longitude").points
    time = data_subset[0].coord("time").points

    no_dims = len(data_subset[0].shape)

    if (no_dims == 4):
        p_mod = data_subset[0].coord("air_pressure").points
        p_units = 'Pa'

    requested_months = (eyear - syear + 1)*12 #how many months span requested time period

    data2 = np.zeros((requested_months,len(p_mod),len(lat),len(lon)))

    nc=ncfile(local_store[0]) # very inefficeint 
    ...
    if (inter == False): out_name = data_path + variable + '_' + vtype + '_' + expid +'_'+experiment+'_'+variant_label+'_'+grid_label+'_'+str(syear)+'01-'+str(eyear)+'12_revised.nc'
    if (inter == True): out_name = data_path + variable + '_' + vtype + '_' + expid +  '_'+experiment+'_'+variant_label+'_common_grid_'+str(syear)+'01-'+str(eyear)+'12_revised.nc'
    ...
    time_cubes = 0
    for loop_cubes in np.arange(len(data_subset)):

        if (model_name != 'FGOALS-g3'): # FGOALS has historical data which extends to 2016 - so messes up the swcript
            data2[time_cubes:time_cubes+data_subset[loop_cubes].shape[0],:,:,:] = data_subset[loop_cubes].data
            time_cubes = time_cubes + data_subset[loop_cubes].shape[0]

        else:
            if (loop_cubes == 5):
                data2[time_cubes-24:time_cubes+data_subset[loop_cubes].shape[0]-24,:,:,:] = data_subset[loop_cubes].data
            if (loop_cubes < 5):
                data2[time_cubes:time_cubes+data_subset[loop_cubes].shape[0],:,:,:] = data_subset[loop_cubes].data
            time_cubes = time_cubes + data_subset[loop_cubes].shape[0]
...
#    variant_label = str(nc.variant_label)
    ...
    # Create name of output file, and check if it has already been created
#    if (inter == False): out_name = data_path + variable + '_' + vtype + '_' + expid +'_'+experiment+'_'+variant_label+'_'+grid_label+'_'+str(syear)+'01-'+str(eyear)+'12_nm.nc'
#    if (inter == True): out_name = data_path + variable + '_' + vtype + '_' + expid +  '_'+experiment+'_'+variant_label+'_common_grid_'+str(syear)+'01-'+str(eyear)+'12_nm.nc'
...
#            continue
    ...
    if (inter == True):
        ...
        for loop_pvals in np.arange(len(p_obs)):
            loc = np.where(np.round(p_mod,1) == p_obs[loop_pvals])[0] # the rounding is needed because sometimes p is, e.g. 850.000001
    ...
    if (mask == True):
        ...
        years = np.repeat(np.arange(1440/12.) + 2019 - 1440/12. + 1,12) # Defines the years in obs array
